...
const QRTrackerControl = () => {
    ...
    const parseMessageType = (message) => {
        ...
        if (lowerMessage.includes('qr') || lowerMessage.includes('qrcode')) return 'qr';
        if (lowerMessage.includes('barcode')) return 'barcode';
        if (lowerMessage.includes('scan') || lowerMessage.includes('scanning')) return 'scan';
        if (lowerMessage.includes('decode') || lowerMessage.includes('decoded')) return 'decode';
        if (lowerMessage.includes('url') || lowerMessage.includes('http')) return 'url';
        if (lowerMessage.includes('text') || lowerMessage.includes('string')) return 'text';
        if (lowerMessage.includes('data') || lowerMessage.includes('payload')) return 'data';
        if (lowerMessage.includes('wifi') || lowerMessage.includes('network')) return 'wifi';
        if (lowerMessage.includes('contact') || lowerMessage.includes('vcard')) return 'contact';
        if (lowerMessage.includes('sms') || lowerMessage.includes('message')) return 'sms';
        if (lowerMessage.includes('email') || lowerMessage.includes('mailto')) return 'email';
        if (lowerMessage.includes('phone') || lowerMessage.includes('tel')) return 'phone';
        if (lowerMessage.includes('location') || lowerMessage.includes('geo')) return 'location';
        ...
        if (lowerMessage.includes('success') || lowerMessage.includes('found')) return 'success';
    ...
    const getMessageTypeColor = (type) => {
        switch (type) {
            case 'qr': return '#FFA500';
            case 'barcode': return '#00BFFF';
            case 'scan': return '#FFA500';
            case 'decode': return '#32CD32';
            case 'url': return '#0066CC';
            case 'text': return '#FFFFFF';
            case 'data': return '#9370DB';
            case 'wifi': return '#00CED1';
            case 'contact': return '#FF69B4';
            case 'sms': return '#00FF00';
            case 'email': return '#FFD700';
            case 'phone': return '#00FA9A';
            case 'location': return '#FF6347';
    ...
    const sendQRCommand = (command, speedValue) => {
        const now = Date.now();
    ...
    const handleActivate = () => {
        ...
        // Send QR_SCAN command with current speed value
        sendQRCommand('QR_SCAN', speedValue);
    };

    const handleDeactivate = () => {
        setDeviceStatus('deactive');
        // Send STOP_QR_SCAN command with current speed value
        sendQRCommand('STOP_QR_SCAN', speedValue);
    };
                ...
                <View style={styles.content}>
                    ...
                    <View style={styles.controlsCard}>
                        ...
                        <View style={styles.sectionHeader}>
                            <Ionicons name="qr-code" size={20} color="#FFA500" />
                            <Text style={styles.sectionTitle}>QR TRACKER</Text>
                        </View>
                    ...
                    <View style={styles.statusCard}>
                        <View style={styles.sectionHeader}>
                            ...
                            <Text style={styles.sectionTitle}>SCAN STATUS</Text>
                        </View>
                        <View style={styles.divider} />
...
export default QRTrackerControl;