...
const GenderDetection = () => {
    ...
    const [detectionStatus, setDetectionStatus] = useState('deactive'); // 'active' or 'deactive'
    const [confidenceThresholdValue, setConfidenceThresholdValue] = useState(50); // Confidence threshold from 0 to 100
    ...
    const parseMessageType = (message) => {
        ...
        if (lowerMessage.includes('gender')) return 'gender';
        if (lowerMessage.includes('male')) return 'male';
        if (lowerMessage.includes('female')) return 'female';
        ...
        if (lowerMessage.includes('confidence')) return 'confidence';
        if (lowerMessage.includes('track') || lowerMessage.includes('tracking')) return 'tracking';
    ...
    const sendGenderCommand = (command, thresholdLevel) => {
        const now = Date.now();
    ...
    const handleActivate = () => {
        ...
        // Send GENDER_TRACK command with current threshold value
        sendGenderCommand('GENDER_TRACK', confidenceThresholdValue);
    };

    const handleDeactivate = () => {
        setDetectionStatus('deactive');
        // Send STOP_GENDER_TRACK command with current threshold value
        sendGenderCommand('STOP_GENDER_TRACK', confidenceThresholdValue);
    };

    const sendConfidenceCommand = (confidence) => {
        const now = Date.now();
        ...
        client.bind(0, () => {
            // Send confidence update if detection is active
            if (detectionStatus === 'active') {
                const commandString = `GENDER_CONFIDENCE,${confidence}`;
                const message = Buffer.from(commandString, 'utf8');
                // Send to hardware on port 5000
                client.send(message, 0, message.length, 5000, '192.168.0.184', (error) => {
                    if (error) {
    ...
    const updateSliderValue = (xPosition) => {
        ...
        setConfidenceThresholdValue(newValue);
        sendConfidenceCommand(newValue);
    };
            ...
            <View style={styles.contentWrapper}>
                <View style={styles.content}>
                    ...
                    <View style={styles.controlsCard}>
                        {/* Gender Detection Controls */}
                        <View style={styles.sectionHeader}>
                            <Ionicons name="people" size={20} color="#FFA500" />
                            <Text style={styles.sectionTitle}>GENDER DETECTION</Text>
                        </View>
                        ...
                        <View style={styles.buttonContainer}>
                            <TouchableOpacity
                                style={[
                                    ...
                                    detectionStatus === 'active' && styles.controlButtonActive
                                ]}
                            ...
                            >
                                <Ionicons 
                                    name="play-circle" 
                                    size={16} 
                                    color={detectionStatus === 'active' ? '#FFA500' : '#FFFFFF'} 
                                />
                                <Text style={[
                                    styles.controlButtonText,
                                    detectionStatus === 'active' && styles.controlButtonTextActive
                                ]}>
                                    Start Detection
                                </Text>
                            </TouchableOpacity>
                            ...
                            <TouchableOpacity
                                style={[
                                    styles.controlButton,
                                    detectionStatus === 'deactive' && styles.controlButtonActive
                                ]}
                            ...
                            >
                                <Ionicons 
                                    name="stop-circle" 
                                    size={16} 
                                    color={detectionStatus === 'deactive' ? '#FFA500' : '#FFFFFF'} 
                                />
                                <Text style={[
                                    styles.controlButtonText,
                                    detectionStatus === 'deactive' && styles.controlButtonTextActive
                                ]}>
                                    Stop Detection
                                </Text>
                            </TouchableOpacity>
                        ...
                        <View style={[styles.sectionHeader, { marginTop: 15 }]}>
                            <Ionicons name="bar-chart" size={20} color="#FFA500" />
                        ...
                        <View style={styles.sliderContainer}>
                            <View style={styles.sliderHeader}>
                                ...
                                <View style={styles.valueBox}>
                                    <Text style={styles.sliderValue}>{confidenceThresholdValue}%</Text>
                                </View>
                            </View>
                            ...
                            >
                                <View style={styles.sliderTrack}>
                                    <View style={[styles.sliderFill, { width: `${confidenceThresholdValue}%` }]} />
                                    <View style={[styles.sliderThumb, { left: `${confidenceThresholdValue}%` }]} />
                                </View>
                            </View>
                        </View>
                    </View>
                            ...
                            {currentHardwareValue ? (
                                ...
                                <View style={styles.dataInactive}>
                                    ...
                                    <Text style={styles.inactiveSubtext}>
                                        ...
                                        Waiting for gender detection data...
                                    </Text>
                                </View>
                            )}
                        </View>
                                ...
                                <View style={styles.statusActive}>
                                    ...
                                    <View style={styles.configInfo}>
                                        <View style={styles.configItem}>
                                            <Ionicons name="bar-chart" size={12} color="#FFA500" />
                                            <Text style={styles.configText}>Confidence: {confidenceThresholdValue}%</Text>
                                        </View>
                                        <View style={styles.configItem}>
