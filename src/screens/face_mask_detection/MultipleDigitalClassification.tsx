...
const MultipleDigitalClassification = () => {
    const insets = useSafeAreaInsets();
    const [systemStatus, setSystemStatus] = useState('deactive'); // 'active' or 'deactive'
    const [accuracyThresholdValue, setAccuracyThresholdValue] = useState(50); // Accuracy threshold from 0 to 100
    ...
    const parseMessageType = (message) => {
        ...
        if (lowerMessage.includes('digit') || lowerMessage.includes('number')) return 'digit';
        if (lowerMessage.includes('classify') || lowerMessage.includes('classification')) return 'classification';
        if (lowerMessage.includes('0') || lowerMessage.includes('zero')) return 'zero';
        if (lowerMessage.includes('1') || lowerMessage.includes('one')) return 'one';
        if (lowerMessage.includes('2') || lowerMessage.includes('two')) return 'two';
        if (lowerMessage.includes('3') || lowerMessage.includes('three')) return 'three';
        if (lowerMessage.includes('4') || lowerMessage.includes('four')) return 'four';
        if (lowerMessage.includes('5') || lowerMessage.includes('five')) return 'five';
        if (lowerMessage.includes('6') || lowerMessage.includes('six')) return 'six';
        if (lowerMessage.includes('7') || lowerMessage.includes('seven')) return 'seven';
        if (lowerMessage.includes('8') || lowerMessage.includes('eight')) return 'eight';
        if (lowerMessage.includes('9') || lowerMessage.includes('nine')) return 'nine';
        if (lowerMessage.includes('accuracy') || lowerMessage.includes('confidence')) return 'accuracy';
        if (lowerMessage.includes('prediction') || lowerMessage.includes('predict')) return 'prediction';
        if (lowerMessage.includes('model') || lowerMessage.includes('neural')) return 'model';
    ...
    const getMessageTypeIcon = (type) => {
        switch (type) {
            case 'digit': return 'calculator';
            case 'classification': return 'library';
            case 'zero': return 'radio-button-off';
            case 'one': return 'finger-print';
            case 'two': return 'duplicate';
            case 'three': return 'triangle';
            case 'four': return 'square';
            case 'five': return 'star';
            case 'six': return 'hexagon';
            case 'seven': return 'flash';
            case 'eight': return 'infinite';
            case 'nine': return 'ellipse';
    ...
    const getMessageTypeColor = (type) => {
        switch (type) {
            case 'digit': return '#FFA500';
            case 'classification': return '#00FF00';
            case 'zero': return '#808080';
            case 'one': return '#FF0000';
            case 'two': return '#00FF00';
            case 'three': return '#0000FF';
            case 'four': return '#FFFF00';
            case 'five': return '#FF00FF';
            case 'six': return '#00FFFF';
            case 'seven': return '#FFA500';
            case 'eight': return '#800080';
            case 'nine': return '#FFC0CB';
            case 'accuracy': return '#9370DB';
            case 'prediction': return '#32CD32';
    ...
    const sendClassificationCommand = (command, thresholdLevel) => {
        const now = Date.now();
    ...
    const handleActivate = () => {
        ...
        // Send DIGIT_DETECT command with current threshold value
        sendClassificationCommand('DIGIT_DETECT', accuracyThresholdValue);
    };

    const handleDeactivate = () => {
        setSystemStatus('deactive');
        // Send STOP_DIGIT_DETECT command with current threshold value
        sendClassificationCommand('STOP_DIGIT_DETECT', accuracyThresholdValue);
    };

    const sendAccuracyCommand = (accuracy) => {
        const now = Date.now();
        ...
        client.bind(0, () => {
            // Send accuracy update if classification is active
            if (systemStatus === 'active') {
                const commandString = `CLASSIFY_ACCURACY,${accuracy}`;
                const message = Buffer.from(commandString, 'utf8');
                // Send to hardware on port 5000
                client.send(message, 0, message.length, 5000, getGlobalIP(), (error) => {
                    if (error) {
    ...
    const updateSliderValue = (xPosition) => {
        ...
        setAccuracyThresholdValue(newValue);
        sendAccuracyCommand(newValue);
    };
            ...
            <View style={styles.contentWrapper}>
                <View style={styles.content}>
                    ...
                    <View style={styles.controlsCard}>
                        {/* Multiple Digital Classification Controls */}
                        <View style={styles.sectionHeader}>
                            <Ionicons name="calculator" size={20} color="#FFA500" />
                            <Text style={styles.sectionTitle}>DIGIT CLASSIFICATION</Text>
                        </View>
                        ...
                        <View style={styles.buttonContainer}>
                            <TouchableOpacity
                                style={[
                                    ...
                                    systemStatus === 'active' && styles.controlButtonActive
                                ]}
                            ...
                            >
                                <Ionicons 
                                    name="play-circle" 
                                    size={16} 
                                    color={systemStatus === 'active' ? '#FFA500' : '#FFFFFF'} 
                                />
                                <Text style={[
                                    styles.controlButtonText,
                                    systemStatus === 'active' && styles.controlButtonTextActive
                                ]}>
                            ...
                            <TouchableOpacity
                                style={[
                                    styles.controlButton,
                                    systemStatus === 'deactive' && styles.controlButtonActive
                                ]}
                            ...
                            >
                                <Ionicons 
                                    name="stop-circle" 
                                    size={16} 
                                    color={systemStatus === 'deactive' ? '#FFA500' : '#FFFFFF'} 
                                />
                                <Text style={[
                                    styles.controlButtonText,
                                    systemStatus === 'deactive' && styles.controlButtonTextActive
                                ]}>
                                    Stop Classification
                                </Text>
                            </TouchableOpacity>
                        ...
                        {/* Accuracy Threshold Controls */}
                        <View style={[styles.sectionHeader, { marginTop: 15 }]}>
                            <Ionicons name="analytics" size={20} color="#FFA500" />
                            <Text style={styles.sectionTitle}>ACCURACY THRESHOLD</Text>
                        </View>
                        <View style={styles.divider} />

                        <View style={styles.sliderContainer}>
                            <View style={styles.sliderHeader}>
                                <Text style={styles.controlLabel}>Accuracy</Text>
                                <View style={styles.valueBox}>
                                    <Text style={styles.sliderValue}>{accuracyThresholdValue}%</Text>
                                </View>
                            </View>

                            {/* Custom Accuracy Threshold Slider */}
                            <View
                            ...
                            >
                                <View style={styles.sliderTrack}>
                                    <View style={[styles.sliderFill, { width: `${accuracyThresholdValue}%` }]} />
                                    <View style={[styles.sliderThumb, { left: `${accuracyThresholdValue}%` }]} />
                                </View>
                            </View>
                        </View>
                    </View>
                                ...
                                <View style={styles.statusActive}>
                                    ...
                                    <View style={styles.configInfo}>
                                        <View style={styles.configItem}>
                                            <Ionicons name="analytics" size={12} color="#FFA500" />
                                            <Text style={styles.configText}>Accuracy: {accuracyThresholdValue}%</Text>
                                        </View>
                                        <View style={styles.configItem}>
...
export default MultipleDigitalClassification;