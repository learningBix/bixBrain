...
const PetDetection = () => {
    ...
    const [recognitionStatus, setRecognitionStatus] = useState('deactive'); // 'active' or 'deactive'
    const [sensitivityThresholdValue, setSensitivityThresholdValue] = useState(50); // Sensitivity threshold from 0 to 100
    ...
    const parseMessageType = (message) => {
        ...
        if (lowerMessage.includes('pet') || lowerMessage.includes('animal')) return 'pet';
        if (lowerMessage.includes('dog') || lowerMessage.includes('canine')) return 'dog';
        if (lowerMessage.includes('cat') || lowerMessage.includes('feline')) return 'cat';
        ...
        if (lowerMessage.includes('recognition') || lowerMessage.includes('recognize')) return 'recognition';
        if (lowerMessage.includes('sensitivity')) return 'sensitivity';
        if (lowerMessage.includes('tracking') || lowerMessage.includes('track')) return 'tracking';
        if (lowerMessage.includes('breed')) return 'breed';
    ...
    const getMessageTypeIcon = (type) => {
        switch (type) {
            ...
            case 'sensitivity': return 'radio-button-on';
            case 'tracking': return 'locate';
            case 'breed': return 'library';
    ...
    const sendPetCommand = (command, thresholdLevel) => {
        const now = Date.now();
    ...
    const handleActivate = () => {
        ...
        // Send PET_DETECT command with current threshold value
        sendPetCommand('PET_DETECT', sensitivityThresholdValue);
    };

    const handleDeactivate = () => {
        setRecognitionStatus('deactive');
        // Send STOP_PET_DETECT command with current threshold value
        sendPetCommand('STOP_PET_DETECT', sensitivityThresholdValue);
    };
    ...
    const sendSensitivityCommand = (sensitivity) => {
        ...
        client.bind(0, () => {
            // Send sensitivity update if recognition is active
            if (recognitionStatus === 'active') {
                const commandString = `PET_SENSITIVITY,${sensitivity}`;
                const message = Buffer.from(commandString, 'utf8');
                // Send to hardware on port 5000
                client.send(message, 0, message.length, 5000, '192.168.0.184', (error) => {
                    if (error) {
    ...
    const updateSliderValue = (xPosition) => {
        ...
        setSensitivityThresholdValue(newValue);
                ...
                <View style={styles.content}>
                    ...
                    <View style={styles.controlsCard}>
                        ...
                        <View style={styles.sectionHeader}>
                            <Ionicons name="paw" size={20} color="#FFA500" />
                            <Text style={styles.sectionTitle}>PET DETECTION</Text>
                        </View>
                        ...
                        <View style={styles.buttonContainer}>
                            <TouchableOpacity
                                style={[
                                    ...
                                    recognitionStatus === 'active' && styles.controlButtonActive
                                ]}
                            ...
                            >
                                <Ionicons 
                                    name="play-circle" 
                                    size={16} 
                                    color={recognitionStatus === 'active' ? '#FFA500' : '#FFFFFF'} 
                                />
                                <Text style={[
                                    styles.controlButtonText,
                                    recognitionStatus === 'active' && styles.controlButtonTextActive
                                ]}>
                                    Start Recognition
                                </Text>
                            </TouchableOpacity>
                            ...
                            <TouchableOpacity
                                style={[
                                    styles.controlButton,
                                    recognitionStatus === 'deactive' && styles.controlButtonActive
                                ]}
                            ...
                            >
                                <Ionicons 
                                    name="stop-circle" 
                                    size={16} 
                                    color={recognitionStatus === 'deactive' ? '#FFA500' : '#FFFFFF'} 
                                />
                                <Text style={[
                                    styles.controlButtonText,
                                    recognitionStatus === 'deactive' && styles.controlButtonTextActive
                                ]}>
                                    Stop Recognition
                                </Text>
                            </TouchableOpacity>
                        ...
                        {/* Recognition Sensitivity Controls */}
                        <View style={[styles.sectionHeader, { marginTop: 15 }]}>
                            <Ionicons name="radio-button-on" size={20} color="#FFA500" />
                        ...
                        <View style={styles.sliderContainer}>
                            <View style={styles.sliderHeader}>
                                ...
                                <View style={styles.valueBox}>
                                    <Text style={styles.sliderValue}>{sensitivityThresholdValue}%</Text>
                                </View>
                            </View>

                            {/* Custom Recognition Sensitivity Slider */}
                            <View
                            ...
                            >
                                <View style={styles.sliderTrack}>
                                    <View style={[styles.sliderFill, { width: `${sensitivityThresholdValue}%` }]} />
                                    <View style={[styles.sliderThumb, { left: `${sensitivityThresholdValue}%` }]} />
                                </View>
                            </View>
                        </View>
                    </View>
                            ...
                            {currentHardwareValue ? (
                                ...
                                <View style={styles.dataInactive}>
                                    ...
                                    <Text style={styles.inactiveSubtext}>
                                        ...
                                        Waiting for pet detection data...
                                    </Text>
                                </View>
                            )}
                        </View>
                                ...
                                <View style={styles.statusActive}>
                                    ...
                                    <View style={styles.configInfo}>
                                        <View style={styles.configItem}>
                                            <Ionicons name="radio-button-on" size={12} color="#FFA500" />
                                            <Text style={styles.configText}>Sensitivity: {sensitivityThresholdValue}%</Text>
                                        </View>
                                        <View style={styles.configItem}>
